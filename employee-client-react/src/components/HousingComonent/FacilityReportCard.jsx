...
function FacilityReportCard({ reportData }) {
  ...
  const {
    comments, status, title, description, createdDatetime, createdBy,
  } = reportData;
  ...
  return (
    <Card sx={{
      minWidth: 275, maxWidth: 800, margin: 'auto', mt: 2,
    }}
    >
      <AddCommentModal />
      <EditCommentModal />
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <CardContent>
          <Typography paragraph align="left">Comments:</Typography>
          <List>
            {comments.map((comment) => (
              <ListItem key={comment.user} alignItems="flex-start">
                ...
                <ListItemText
                  ...
                  secondary={(
                    <>
                      <Typography component="span" variant="body2" color="text.primary" display="block" align="left">
                        {comment.description}
                        <br />
                        *Will conditional Button after connecting api*
                        <Button onClick={() => handleEditComment(comment.description)}>
                          Edit
                        </Button>
                      </Typography>
        ...
        <CardActions>
          <Button size="small" variant="contained" onClick={handleCommentReport}>Comment</Button>
          <Button size="small" variant="outlined" onClick={handleCloseReport} sx={{ marginLeft: 'auto' }}>Close Report</Button>
        </CardActions>
      </Collapse>
    </Card>
  );
